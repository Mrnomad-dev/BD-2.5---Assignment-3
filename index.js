...
//function for price
//------------high-to-kow
function sortPriceHighToLow(hotel1, hotel2) {
  return hotel2.price - hotel1.price;
}
//------------low-to-high
function sortPriceLowToHigh(hotel1, hotel2) {
  return hotel1.price - hotel2.price;
}
//Endpoint 1.1: Get the hotels sorted by pricing
app.get('/hotels/sort/pricing', (req, res) => {
  let hotelCopy = hotels.slice();
  let pricing = req.query.pricing;

  if (pricing === 'low-to-high') {
    hotelCopy.sort(sortPriceLowToHigh);
  } else {
    hotelCopy.sort(sortPriceHighToLow);
  }
...
//function  based on their Ratings
//------------high-to-kow
function sortRatingHighToLow(rating1, rating2) {
  return rating2.rating - rating1.rating;
}
//------------low-to-high
function sortRatingLowToHigh(rating1, rating2) {
...
app.get('/hotels/sort/rating', (req, res) => {
  ...
  if (rating === 'low-to-high') {
    hotelCopy.sort(sortRatingLowToHigh);
  } else {
    hotelCopy.sort(sortRatingHighToLow);
  }
  res.json({ hotels: hotelCopy });
});
...
//----------least-to-most
function sortHotelLeastToMost(review1, review2) {
  return review1.reviews - review2.reviews;
}
//----------most-to-least
function sortHotelMostToLeast(review1, review2) {
...
app.get('/hotels/sort/reviews', (req, res) => {
  ...
  if (reviews === 'least-to-most') {
    hotelCopy.sort(sortHotelLeastToMost);
  } else {
    hotelCopy.sort(sortHotelMostToLeast);
  }
  res.json({ hotels: hotelCopy });
});
...
app.get('/hotels/filter/amenity', (req, res) => {
  ...
  let checkAmenity = hotels.filter((hotel) => filterByAmenity(hotel, amenity));
  res.json({ hotels: checkAmenity });
});
//hotels/filter/amenity?amenity=Spa
//hotels/filter/amenity?amenity=Bar
//hotels/filter/amenity?amenity=Pool
//hotels/filter/amenity?amenity=Restaurant
//hotels/filter/amenity?amenity=Gym
//hotels/filter/amenity?amenity=Pet Friendly
//hotels/filter/amenity?amenity=Parking
//hotels/filter/amenity?amenity=Free WiFi
...
app.get('/hotels/filter/country', (req, res) => {
  ...
  let checkCountry = hotels.filter((hotel) => filterByCountry(hotel, country));
  res.json({ hotels: checkCountry });
});
//hotels/filter/country?country=india
//hotels/filter/country?country=France
//hotels/filter/country?country=USA
//hotels/filter/country?country=India
//hotels/filter/country?country=Germany
//hotels/filter/country?country=United Kingdom
//hotels/filter/country?country=Australia
//hotels/filter/country?country=South Africa
...
app.get('/hotels/filter/category', (req, res) => {
  ...
  let checkCategory = hotels.filter((hotel) =>
    filterByCategory(hotel, category)
  );
  res.json({ hotels: checkCategory });
});
//hotels/filter/category?category=Mid-Range
//hotels/filter/category?category=Family
//hotels/filter/category?category=Luxury
//hotels/filter/category?category=Boutique
//hotels/filter/category?category=Resort
//hotels/filter/category?category=Budget
